<template>
  <div style="padding: 1%">
    <div class="inspection-header content-header">
      ...
      <el-button
          style="  width: 9.375rem;background-color: #15B3B4 !important;"
          icon="el-icon-plus"
          type="success"
          size="mini"
          @click="addTimedTask(null, 1)"
          >{{ $t('plan_config.addTimedTask_label') }}</el-button
        >
    ...
    >
      ...
      >
        ...
        <el-form-item label="计划类型" prop="PlanType">
          ...
          style="width:12vw"
            placeholder="请选择计划类型"
            :disabled="taskForm.isDetail || taskForm.isEdit"
            v-model="taskForm.PlanType"
            clearable
          >
            <el-option
              v-for="item in options"
        ...
        <el-form-item label="巡检计划模板" prop="TaskTemplateID">
          ...
          style="width:12vw"
            :disabled="taskForm.isDetail"
            v-model="taskForm.TaskTemplateID"
            placeholder="请选择巡检计划模板"
            clearable
          >
            <el-option
        ...
        <el-form-item v-if="selectRobotFlag" label="机器人" prop="CarrierID">
          ...
          <template v-if="taskForm.PlanType !== 3">
            ...
            <el-form-item label="执行次数" prop="ExecuteCount">
              ...
              style="width:12vw"
                ...
                v-model.number="taskForm.ExecuteCount"
                :disabled="taskForm.isDetail"
              ></el-input>
            </el-form-item>
            <!-- oninput="if(value.length==1){value=value.replace(/[^1-9]/g,'')}else{value=value.replace(/\D/g,'')}" -->
            <el-form-item label="执行间隔(分钟)" prop="ExecuteSpace" >
              <el-input  
              style="width:12vw"
                v-model.number="taskForm.ExecuteSpace"
                :disabled="taskForm.isDetail"
              ></el-input>
            </el-form-item>
            <el-form-item 
              v-if="taskForm.PlanType !== 2"
              label="执行时间"
              prop="OpenTime"        
            >
              <el-time-picker  
            ...
            >
              ...
              style="width:12vw"
                ...
                placeholder="请选择执行频率"
                clearable
                @change="frequencyTypeChange(taskForm.FrequencyType)"
              >
                <el-option
              ...
              <template>
                <template v-if="taskForm.FrequencyType == 2">
                  <el-select
                    :disabled="taskForm.isDetail"
                    placeholder="请选择执行日期"
                    v-model="taskForm.FrequencyCount"
                    clearable
                  >
                    <el-option
                ...
                <template v-else-if="taskForm.FrequencyType == 3">
                  ...
                  >
                    <el-option
                      v-for="(item,
                      index) in '1111111111111111111111111111111'.split('')"
                      :key="index"
                      :label="index + 1 + '号'"
                      :value="index + 1"
                    >
          ...
          <template v-else>
            ...
            <el-form-item label="开始时间" prop="StartTime" >
              <el-date-picker style="width:205px"
                :disabled="taskForm.isDetail"
        ...
        <el-form-item v-if="taskForm.PlanType === 1">
          <span style="color:#f56c6c;margin-left:55px"
            >同一机器人设置启动时间间隔不能小于1小时</span
          >
        </el-form-item>
